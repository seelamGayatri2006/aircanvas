    ...
    def draw(self, img, hover_pt):
        ...
        for i in range(num_colors):
            ...
            if i == self.selected_index:
                shift = 15
                cv2.ellipse(img,(cx, cy),(radius + shift, radius + shift),0,start_ang,end_ang,(255,255,255),-1)

            if i == hover_index:
                thickness += 10
                cv2.putText(img,name,(cx - 40, cy + radius + 100),cv2.FONT_HERSHEY_SIMPLEX,1,color,2)

            cv2.ellipse(img,(cx, cy),(radius + shift + (thickness//2),radius + shift + (thickness//2)),0,start_ang,end_ang,color,thickness)
...
def main():
    ...
    # Read first frame to get camera size
    ret, frame = cap.read()
    h, w, _ = frame.shape
    ...
    while True:
        ...
        blur_up = cv2.resize(blur,(canvas.shape[1],canvas.shape[0]))
        ...
        final_canvas = cv2.resize(final_canvas,(img_bg.shape[1],img_bg.shape[0]))
